export const plans = [
  {
    id: 1,
    title: "Kerala Dream Escape",
    location: "Munnar + Alleppey + Kochi",
    days: 5,
    nights: 4,
    category: "Nature",
    image:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT8DWpSKp3wxiMc4uz38BhH_Pdmjqv1avxC25V4Jb4QVw&s=10",

    places: [
      "Munnar Tea Gardens",
      "Eravikulam National Park",
      "Alleppey Houseboat",
      "Fort Kochi",
    ],

    itinerary: [
      {
        day: 1,
        title: "Arrival in Munnar",
        activities: [
          "Arrive in Munnar",
          "Check-in to hotel",
          "Visit Munnar Tea Gardens",
          "Evening at leisure",
        ],
      },
      {
        day: 2,
        title: "Explore Munnar",
        activities: [
          "Visit Eravikulam National Park",
          "Visit Mattupetty Dam",
          "Visit Echo Point",
          "Return to hotel",
        ],
      },
      {
        day: 3,
        title: "Munnar to Alleppey",
        activities: [
          "Travel to Alleppey",
          "Check-in to houseboat",
          "Backwater cruise",
          "Traditional Kerala dinner",
        ],
      },
      {
        day: 4,
        title: "Alleppey to Kochi",
        activities: [
          "Breakfast",
          "Travel to Kochi",
          "Explore Fort Kochi",
          "Visit Chinese Fishing Nets",
        ],
      },
      {
        day: 5,
        title: "Kochi Departure",
        activities: [
          "Visit Mattancherry Palace",
          "Explore Jew Town",
          "Shopping",
          "Departure",
        ],
      },
    ],
  },

  {
    id: 2,
    title: "Tamil Nadu Temple Trail",
    location: "Chennai + Mahabalipuram + Thanjavur + Madurai + Rameswaram",
    days: 7,
    nights: 6,
    category: "Spiritual",
    image:
      "https://images.unsplash.com/photo-1600100397608-f0103f7f2f18?auto=format&fit=crop&w=1200&q=80",

    places: [
      "Marina Beach",
      "Shore Temple",
      "Brihadeeswarar Temple",
      "Meenakshi Amman Temple",
      "Ramanathaswamy Temple",
    ],

    itinerary: [
      {
        day: 1,
        title: "Explore Chennai",
        activities: [
          "Arrive in Chennai",
          "Visit Marina Beach",
          "Visit Kapaleeshwarar Temple",
          "Explore local food",
        ],
      },
      {
        day: 2,
        title: "Chennai to Mahabalipuram",
        activities: [
          "Travel to Mahabalipuram",
          "Visit Shore Temple",
          "Explore Pancha Rathas",
          "Beach evening",
        ],
      },
      {
        day: 3,
        title: "Mahabalipuram to Thanjavur",
        activities: [
          "Travel to Thanjavur",
          "Visit Brihadeeswarar Temple",
          "Visit Thanjavur Palace",
          "Local dinner",
        ],
      },
      {
        day: 4,
        title: "Thanjavur to Madurai",
        activities: [
          "Travel to Madurai",
          "Visit Meenakshi Amman Temple",
          "Explore local markets",
          "Evening temple visit",
        ],
      },
      {
        day: 5,
        title: "Explore Madurai",
        activities: [
          "Visit Thirumalai Nayakkar Palace",
          "Visit Gandhi Museum",
          "Taste traditional Madurai food",
        ],
      },
      {
        day: 6,
        title: "Madurai to Rameswaram",
        activities: [
          "Travel to Rameswaram",
          "Visit Ramanathaswamy Temple",
          "Visit Pamban Bridge",
          "Visit Agni Theertham",
        ],
      },
      {
        day: 7,
        title: "Dhanushkodi and Departure",
        activities: [
          "Visit Dhanushkodi",
          "Explore the coastline",
          "Return to Rameswaram",
          "Departure",
        ],
      },
    ],
  },

  {
    id: 3,
    title: "Karnataka Explorer",
    location: "Bengaluru + Mysore + Coorg + Hampi",
    days: 7,
    nights: 6,
    category: "Heritage",
    image:
      "https://images.unsplash.com/photo-1592639296346-560c37a0f711?auto=format&fit=crop&w=1200&q=80",

    places: [
      "Bangalore Palace",
      "Mysore Palace",
      "Coorg Coffee Estates",
      "Abbey Falls",
      "Hampi Ruins",
    ],

    itinerary: [
      {
        day: 1,
        title: "Explore Bengaluru",
        activities: [
          "Arrive in Bengaluru",
          "Visit Bangalore Palace",
          "Explore Cubbon Park",
          "Visit Vidhana Soudha",
        ],
      },
      {
        day: 2,
        title: "Bengaluru to Mysore",
        activities: [
          "Travel to Mysore",
          "Visit Mysore Palace",
          "Explore Devaraja Market",
          "Evening city tour",
        ],
      },
      {
        day: 3,
        title: "Mysore",
        activities: [
          "Visit Chamundi Hills",
          "Visit Mysore Zoo",
          "Explore Brindavan Gardens",
        ],
      },
      {
        day: 4,
        title: "Mysore to Coorg",
        activities: [
          "Travel to Coorg",
          "Visit coffee plantations",
          "Visit Raja's Seat",
          "Relax at resort",
        ],
      },
      {
        day: 5,
        title: "Explore Coorg",
        activities: [
          "Visit Abbey Falls",
          "Visit Dubare Elephant Camp",
          "Coffee plantation experience",
        ],
      },
      {
        day: 6,
        title: "Coorg to Hampi",
        activities: [
          "Travel to Hampi",
          "Visit Hampi Bazaar",
          "Explore Virupaksha Temple",
        ],
      },
      {
        day: 7,
        title: "Explore Hampi and Departure",
        activities: [
          "Visit Vijaya Vittala Temple",
          "Visit Matanga Hill",
          "Explore Hampi ruins",
          "Departure",
        ],
      },
    ],
  },
];